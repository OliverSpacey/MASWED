document.addEventListener('DOMContentLoaded', function () {
    ...
    function createFieldSet() {
        ...
        newRow.innerHTML = `
          <hr>
          <h3>Guest ${rowCount + 1}</h3>
          <label for="firstName${rowCount}">Forename:</label>
          <input type="text" id="firstName${rowCount}" name="first name" placeholder="Forename..." required>
          <p></p>
          <label for="lastName${rowCount}">Surname:</label>
          <input type="text" id="lastName${rowCount}" name="last name" placeholder="Surname..." required>
          <p></p>
        ...
        if (fieldContainer.querySelectorAll('.fieldForms').length >= 9) {
    ...
    function updateGuestNumbers() {
        ...
        allGuests.forEach((row, index) => {
          ...
          if (heading) {
            heading.textContent = `Guest ${index + 2}`;
          }
        });
    }
